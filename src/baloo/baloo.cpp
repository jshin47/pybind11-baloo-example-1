...
PYBIND11_MODULE(baloo, m) {
    ...
    py::class_<OrderBookSnapshot>(m, "OrderBookSnapshot")
        ...
        .def("apply_and_bucket", &OrderBookSnapshot::applyAndBucket, py::return_value_policy::take_ownership, py::arg("updates"), py::arg("time_buckets"), py::arg("bins"), py::arg("ignore_deltas_before_beginning_of_first_bin") = true, "Calculates the bid-ask spread over time buckets")
